...
- Selective Repeat 작동 순서
  ...
  5. 이후 수신 측에서는 2번째 패킷을 받지 못했지만 sequence number가 3인 패킷을 받고 이를 버퍼에 저장하고 ACK3 메세지를 전송 측으로 전송
  
  6. sequence number가 2인 패킷을 아직 받지 못하였기 때문에 제일 마지막으로 받은 패킷에 대한 ACK메세지(ACK1)을 전송
